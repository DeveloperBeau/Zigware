...
encrypt/decrypt round-trip on the Zig side, with a Vue 3 + Vite frontend.
...
From the repo root — bundles the Vue app with Vite into a single `app.js`,
embeds it, and opens the macOS window:

```sh
zig build run-crypto-vue
```
...
crypto-vue/
  ...
  frontend/src/App.vue     the Vue component
```
